...
def move_to_position_cart(x, y, z):
    ...
    if y == 0:
        ...
        theta_base = 90 - degrees(atan(x / y))  # add 2 for backlash
    ...
    # Adjust shoulder for height
    if z != l0:
        theta_shoulder += degrees(atan((z - l0) / r))

    # Add servo-mount compensation
    theta_elbow += 5
    theta_wrist += 5
...
def backlash_compensation_base(theta_base):
    ...
    compensation_value_CW  = 8                        # degrees CW
    ...
    if delta_theta_base > 1:
        if theta_base <= 45:
            ...
            theta_base_comp = theta_base + compensation_value_CCW[idx]
